...
Mutex EmptySeries::_mutex;

SeriesAbstrPtr SeriesImpl::shiftRight(unsigned int n) const {
  return _cache->findAndAdd(MakeShiftRightSeries(*this, n));
}

SeriesAbstrPtr SeriesImpl::shiftLeft(unsigned int n) const {
...
/*SeriesAbstrPtr SeriesImpl::AroonOscillator( const SeriesImpl& series, unsigned
int period ) const
{
  return new AroonOscillatorSeries( series, period );
}
*/
...
/*SeriesAbstrPtr SeriesImpl::MACDFix( const SeriesImpl& series ) const
{
  return new MACDFixSeries( series );
}
*/
...
SeriesAbstrPtr SeriesImpl::PPO(unsigned int fastPeriod, unsigned int slowPeriod,
                               MAType maType) const {
  return _cache->findAndAdd(
      MakePPOSeries(*this, fastPeriod, slowPeriod, maType));
}
...
SeriesAbstrPtr SeriesImpl::Correlation(SeriesAbstrPtr series,
                                       unsigned int period) const {
  return _cache->findAndAdd(MakeCorrelationSeries(*this, *series, period));
}

SeriesAbstrPtr SeriesImpl::Beta(SeriesAbstrPtr series,
                                unsigned int period) const {
...
SeriesAbstrPtr SeriesImpl::MACD(unsigned int fastPeriod,
                                unsigned int slowPeriod,
                                unsigned int signalPeriod) const {
  return _cache->findAndAdd(
      MakeMACDSeries(*this, fastPeriod, slowPeriod, signalPeriod));
}

SeriesAbstrPtr SeriesImpl::MACDSignal(unsigned int fastPeriod,
                                      unsigned int slowPeriod,
                                      unsigned int signalPeriod) const {
  return _cache->findAndAdd(
      MakeMACDSignalSeries(*this, fastPeriod, slowPeriod, signalPeriod));
}

SeriesAbstrPtr SeriesImpl::MACDHist(unsigned int fastPeriod,
                                    unsigned int slowPeriod,
                                    unsigned int signalPeriod) const {
  return _cache->findAndAdd(
      MakeMACDHistSeries(*this, fastPeriod, slowPeriod, signalPeriod));
}

SeriesAbstrPtr SeriesImpl::MACDExt(unsigned int fastPeriod, MAType fastMAType,
                                   unsigned int slowPeriod, MAType slowMAType,
                                   unsigned int signalPeriod,
                                   MAType signalMAType) const {
  return _cache->findAndAdd(MakeMACDExtSeries(*this, fastPeriod, fastMAType,
                                              slowPeriod, slowMAType,
                                              signalPeriod, signalMAType));
}

SeriesAbstrPtr SeriesImpl::MACDSignalExt(
  ...
  return _cache->findAndAdd(
      MakeMACDSignalExtSeries(*this, fastPeriod, fastMAType, slowPeriod,
                              slowMAType, signalPeriod, signalMAType));
}

SeriesAbstrPtr SeriesImpl::MACDHistExt(
    unsigned int fastPeriod, MAType fastMAType, unsigned int slowPeriod,
    MAType slowMAType, unsigned int signalPeriod, MAType signalMAType) const {
  return _cache->findAndAdd(MakeMACDHistExtSeries(*this, fastPeriod, fastMAType,
                                                  slowPeriod, slowMAType,
                                                  signalPeriod, signalMAType));
}
...
SeriesAbstrPtr SeriesImpl::APO(unsigned int fastPeriod, unsigned int slowPeriod,
                               MAType maType) const {
  return _cache->findAndAdd(
      MakeAPOSeries(*this, fastPeriod, slowPeriod, maType));
}
...
SeriesAbstrPtr SeriesImpl::StochRSIFastK(int period, int fastKPeriod,
                                         int fastDPeriod,
                                         MAType fastDMAType) const {
  return _cache->findAndAdd(MakeStochRSIFastKSeries(*this, period, fastKPeriod,
                                                    fastDPeriod, fastDMAType));
}
SeriesAbstrPtr SeriesImpl::StochRSIFastD(int period, int fastKPeriod,
                                         int fastDPeriod,
                                         MAType fastDMAType) const {
  return _cache->findAndAdd(MakeStochRSIFastDSeries(*this, period, fastKPeriod,
                                                    fastDPeriod, fastDMAType));
}
...
tradery::SeriesAbstrPtr SeriesAbstr::create() {
  return SeriesAbstrPtr(new EmptySeries());
}

tradery::SeriesAbstrPtr SeriesAbstr::create(size_t size) {
  return SeriesAbstrPtr(new EmptySeries(size));
}

// global operators allowing using a constant as the first operand in an
// operation ( value + series etc)
#define OPERATOR_VALUE_SERIES(OP)                                             \
  CORE_API Series tradery::operator OP(double value, const Series& series) \
{ \
    std::auto_ptr<SeriesImpl> si(new SeriesImpl());                           \
                                                                              \
    const std::vector<double>& v = series.getVector();                        \
                                                                              \
    for (size_t n = 0; n < series.unsyncSize(); n++)                          \
      si->push_back(value OP v[n]);                                           \
                                                                              \
    si->synchronize(series.synchronizer());                                   \
                                                                              \
    return Series(si.release());                                              \
  \
}

OPERATOR_VALUE_SERIES(+)
OPERATOR_VALUE_SERIES(-)
OPERATOR_VALUE_SERIES(/)
OPERATOR_VALUE_SERIES(*)

CORE_API std::ostream& Series::dump(std::ostream& os, size_t k) const {
  std::ostringstream o;
  for (size_t n = 0; n < min2(size(), k); n++) o << (*this)[n] << std::endl;

  sprint(o, os);
  return os;
}

CORE_API std::ostream& Series::dumpUnsynced(std::ostream& os, size_t k) const {
  std::ostringstream o;

  const std::vector<double>& v = getVector();

  for (size_t n = 0; n < min2(v.size(), k); n++) o << v[n] << std::endl;
