...
typedef ManagedPtr<TradeAndPosition> TradeAndPositionPtr;

typedef std::multimap<Date, TradeAndPositionPtr> DatesToTradesMap;
...
typedef std::map<std::string, Trades> StrToTradesMap;

class SymbolTradesException {};

typedef std::pair<Trades::iterator, Trades::iterator> TradesRange;
...
class SymbolTrades {
 ...
 public:
  ...
  std::auto_ptr<TradesRange> getExits(const std::string& symbol,
                                      const Date& date) {
    ...
    if (i != _exits.end()) {
      ...
      if (i != trades.end())
        return std::auto_ptr<TradesRange>(
            new TradesRange(trades.find(date), trades.upper_bound(date)));
      else
        return std::auto_ptr<TradesRange>(new TradesRange(i, i));
    } else {
      // the position is still opened, continue
      return std::auto_ptr<TradesRange>(0);
    }
  ...
  ExternalTrades(const std::string& fileName) {
    ...
    if (extFile) {
      do {
        ...
        TradeAndPositionPtr trade(new TradeAndPosition(str));
        _st.add(trade);
      } while (!extFile.eof());
    }
  }
...
class ExtTradesSystem : public BarSystem<ExtTradesSystem> {
  ...
  ManagedPtr<ExternalTrades> _trades;
 ...
 public:
  ExtTradesSystem(const std::vector<std::string>* params = 0)
    ...
    if (params != 0) _trades = new ExternalTrades((*params)[0]);
  }
 ...
 private:
  ...
  void onBar(Index bar) {
    ...
    for (Trades::iterator i = entries.first; i != entries.second; i++) {
      ...
      if (tp.isLong()) {
        ...
        if (id != 0)
          ...
          os << "Cannot open long position - entry price " << tp.entryPrice()
             << " may be out of price range for bar "
             << date.to_iso_extended_string();
          systemError(os.str());
        }
      } else {
        //        PRINT_LINE( "20" );
        ...
        if (id != 0)
          ...
          std::ostringstream os;
          os << "Cannot open short position - entry price " << tp.entryPrice()
             << " may be out of price range for bar "
             << date.to_iso_extended_string();
          systemError(os.str());
        }
    ...
    std::auto_ptr<TradesRange> x(_trades->getExits(__super::getSymbol(), date));
    if (x.get() != 0) {
      TradesRange& exits(*x);
      for (Trades::iterator i = exits.first; i != exits.second; i++) {
        ...
        if (tp.isLong()) {
          ...
          if (!b) {
            std::ostringstream os;
            os << "Cannot close long position - exit price " << tp.exitPrice()
               << " may be out of price range for bar "
               << date.to_iso_extended_string();
            systemError(os.str());
          }
          ...
          if (!b) {
            std::ostringstream os;
            os << "Cannot close short position - exit price " << tp.exitPrice()
               << " may be out of price range for bar "
               << date.to_iso_extended_string();
            systemError(os.str());
          }
