...
class BuildErrorsParser {
 private:
  class BuildEvent {
   ...
   public:
    BuildEvent(const std::string& fileName, const std::string& line,
               const std::string& type, const std::string& errorCode,
        ...
        : _fileName(fileName),
          ...
          _message(message) {
      //      cout << _fileName << ", " << _line << _T( ", " ) << _message <<
      //      std::endl;
    ...
    std::string toString() const {
      ...
      o << "<td>" << (_line) << "</td><td>"
        << (_type == warning ? "Warning" : "Error") << "</td><td>" << _message
        << "</td>" << std::endl;
      //      std::cout << o.str();
      return o.str();
    }
  ...
  typedef ManagedPtr<BuildEvent> BuildEventPtr;

  class BuildEvents : public std::vector<BuildEventPtr> {
   public:
    unsigned long write(std::ofstream& ofs, bool onePerLine,
                        unsigned long max) const {
      std::ostringstream os;
      os << "<table class=\"list_table\">" << std::endl
         << "<tr><th>Line</th><th>Type</th><th>Message</th></tr>" << std::endl;

      unsigned long lastLine = 0;
      unsigned long count = 0;
      bool empty = true;
      for (__super::const_iterator i = __super::begin();
           i != __super::end() && count < max; i++, count++) {
        BuildEventPtr event = *i;
        LOG(log_info, "Event: " << event->toString());
        if (event->line() != lastLine) {
          if (empty) ofs << os.str();
          empty = false;
          ofs << "<tr class=\"" << (count % 2 ? "d0\"" : "d1\"") << ">"
              << std::endl;
          ofs << event->toString();
          ofs << "</tr>" << std::endl;
          if (onePerLine) lastLine = event->line();
        }
      }
      if (!empty) ofs << "</table>" << std::endl;
  ...
  class SystemBuildEvents {
   ...
   public:
    ...
    void write(std::ofstream& ofs, bool onePerLine, unsigned long max) const {
      if (_events.size() > 0) {
        ofs << "<br>Messages for system <b>"
            << (_systemName.empty() ? "[unnamed]" : _systemName) << "</b><br>"
            << std::endl;
 ...
 public:
  BuildErrorsParser(std::istream& is, SessionContextPtr context)
      : _is(is),
        systemNameKeyword(context->getConfig()->getSystemNameKeyword()),
        classNameKeyword(context->getConfig()->getClassNameKeyword()) {
  ...
  void parse() {
    ...
    do {
      ...
      if (!line.empty()) {
        ...
        if (!className.empty()) parseLine(line, className, sbe);
      }
    } while (!_is.eof());
#ifdef REGEX_TRACE
  ...
  void cleanKeyword(std::string& line, const std::string& keyword) {
    //		COUT << _T( "line: " ) << line << _T( ", keyword: " ) << keyword
    //<<  std::endl;
    std::string::size_type ix = line.find(keyword);
    if (ix != std::string::npos) {
      //      std::cout << _T( "cleaning line: " ) << line << std::endl;
      line.erase(ix, keyword.length());
      //      std::cout << _T( "cleaned line: " ) << line << std::endl;
    }
  }

  void replaceKeyword(std::string& line, const std::string& keyword,
  ...
  void cleanLine(std::string& line, const std::string& className,
                 unsigned int code) {
  ...
  void parseLine(const std::string& line, const std::string& className,
    ...
    try {
      ...
      // expression: (.+)\((\d+\) : (\w+) (C\d+): (.+)
      static const boost::regex CompilerErrorRegex(
          "(.+)\\((\\d+)\\) : ((?:\\w| )+) (C\\d+): (.+)");
      std::string::const_iterator start, end;
      start = line.begin();
      end = line.end();
      ...
      if (boost::regex_search(start, end, what, CompilerErrorRegex, flags)) {
        ...
        sbe->add(BuildEventPtr(
            new BuildEvent(what[1], what[2], what[3], what[4], l)));
      }
