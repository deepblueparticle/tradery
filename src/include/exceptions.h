...
class DataIndexOutOfRangeException : public CoreException {
 ...
 public:
  DataIndexOutOfRangeException(size_t size, size_t index,
                               const std::string& symbol)
      : _size(size),
        _index(index),
        CoreException(DATA_INDEX_OUT_OF_RANGE_ERROR),
        _symbol(symbol) {
    std::ostringstream _o;
    _o << "Index out of range exception - size: " << _size
       << ", index: " << _index << " on: " << _symbol;
    setMessage(_o);
  }
...
class TickIndexOutOfRangeException : public DataIndexOutOfRangeException {
 public:
  TickIndexOutOfRangeException(size_t size, size_t index,
                               const std::string& symbol)
...
class BarIndexOutOfRangeException : public DataIndexOutOfRangeException {
 public:
  BarIndexOutOfRangeException(size_t size, size_t index,
                              const std::string& symbol)
      : DataIndexOutOfRangeException(size, index, symbol) {}
};
...
class SeriesIndexOutOfRangeException : public CoreException {
 ...
 public:
  ...
  SeriesIndexOutOfRangeException(size_t size, size_t index)
      : _size(size),
        _index(index),
        CoreException(SERIES_INDEX_OUT_OF_RANGE_ERROR) {
    std::ostringstream _o;
    _o << "Series index out of range - size: " << _size
       << ", index: " << _index;
    setMessage(_o);
  }
...
class SynchronizedSeriesIndexOutOfRangeException : public CoreException {
 ...
 public:
  SynchronizedSeriesIndexOutOfRangeException(size_t size, size_t index)
      : _size(size),
        _index(index),
        CoreException(SYNCHRONIZED_SERIES_INDEX_OUT_OF_RANGE_ERROR) {
    std::ostringstream _o;
    _o << "Synchronized series index out of range - size: " << _size
       << ", index: " << _index;
    setMessage(_o);
  }
...
class TimeSeriesIndexOutOfRangeException : public CoreException {
 ...
 public:
  TimeSeriesIndexOutOfRangeException(size_t size, size_t index)
      : _size(size),
        _index(index),
        CoreException(TIME_SERIES_INDEX_OUT_OF_RANGE_ERROR) {
    std::ostringstream _o;
    _o << "Time Series index out of range - size: " << _size
       << ", index: " << _index;
    setMessage(_o);
  }
...
class OperationNotAllowedOnSynchronizedseries : public CoreException {
 public:
  OperationNotAllowedOnSynchronizedseries()
...
class OperationOnSeriesSyncedToDifferentSynchronizers : public CoreException {
 public:
  OperationOnSeriesSyncedToDifferentSynchronizers()
      : CoreException(OPERATION_ON_SERIES_SYNCED_TO_DIFFERENT_SYNCHRONIZERS,
                      "Error trying to perform an operation between 2 "
                      "synchronized series") {}
};
...
class OperationOnUnequalSizeSeriesException : public CoreException {
 ...
 public:
  OperationOnUnequalSizeSeriesException(size_t size1, size_t size2)
      : _size1(size1),
        _size2(size2),
        CoreException(OPERATION_ON_UNEQUAL_SIZE_SERIES_ERROR) {
    std::ostringstream _o;
    _o << "Operation on series of unequal sizes - size1: " << _size1
       << ", size2: " << _size2;
    setMessage(_o);
  }
  /**
...
class InvalidIndexForOperationException : public CoreException {
 ...
 public:
  InvalidIndexForOperationException(size_t index,
                                    const std::string& operationName)
      : _index(index),
        _operationName(operationName),
        CoreException(INVALID_INDEX_FOR_OPERATION_EXCEPTION) {
    std::ostringstream o;
...
class ClosingAlreadyClosedPositionException : public CoreException {
 public:
  ...
  ClosingAlreadyClosedPositionException()
      : CoreException(CLOSING_ALREADY_CLOSED_POSITION_ERROR,
...
class DataSourceException : public CoreException {
 ...
 public:
   ...
   * @see ErrorCode
   */
  DataSourceException(ErrorCode errorCode, const std::string& message,
 ...
 public:
  DataSourceAlreadyRegisteredException(const std::string& dataSourceName)
      : _dataSourceName(dataSourceName),
        CoreException(DATA_SOURCE_ALREADY_REGISTERED_ERROR,
                      std::string("Data source already registered: ") +
                          dataSourceName + "\n") {}
...
class DataSourceNotRegisteredException : public CoreException {
 ...
 public:
  DataSourceNotRegisteredException(const std::string& dataSourceName)
      : _dataSourceName(dataSourceName),
        CoreException(DATA_SOURCE_NOT_REGISTERED_ERROR,
                      std::string("Data source not registered: ") +
                          dataSourceName + "\n") {}
...
class SchedulerReentrantRunCallException : public CoreException {
 public:
  ...
  SchedulerReentrantRunCallException()
      : CoreException(SCHEDULER_REENTRANT_CALL_ERROR,
                      "Scheduler reentrant run call not allowed") {}
};
...
class PositionCloseOperationOnOpenPositionException : public CoreException {
 public:
  PositionCloseOperationOnOpenPositionException(const std::string& message)
      : CoreException(POSITION_CLOSE_OPERATION_ON_OPEN_POSITION_ERROR,
                      "Cannot perform an operation that require a closed "
                      "position on a still open position - test if the "
...
class PositionZeroPriceException : public CoreException {
 public:
  PositionZeroPriceException()
      : CoreException(POSITION_ZERO_PRICE_ERROR,
                      "A position entry or exit price is 0") {}
};
...
class ExitRunnableException : public CoreException {
 public:
  ExitRunnableException(const std::string& exitMessage = "")
      : CoreException(
            EXIT_STATEMENT_CALL_EXCEPTION,
            std::string("System stopped due to exit() call: ") + exitMessage) {}
};

class InvalidBarsCollectionException : public CoreException {
 public:
  InvalidBarsCollectionException(const std::string& symbol)
      : CoreException(INVALID_BARS_COLLECTION_EXCEPTION,
                      std::string("Trying to use an invalid bars collection. "
                                  "Check if data is available for \"")
                          << symbol << "\"") {}
};
...
class InvalidPositionException : public CoreException {
 public:
  InvalidPositionException()
      : CoreException(INVALID_POSITION_EXCEPTION,
                      "Trying to use an invalid Position object") {}
};

class ClosingPostionOnDifferentSymbolException : public CoreException {
 public:
  ClosingPostionOnDifferentSymbolException(const std::string& originalSymbol,
                                           const std::string& newSymbol)
      : CoreException(
            CLOSING_POSITION_ON_DIFFERENT_SYMBOL_EXCEPTION,
            std::string(
                "Closing position on different symbol. Original symbol: ") +
                originalSymbol + ", new symbol: " + newSymbol) {}
};
