 ...
 */
typedef std::auto_ptr<Slippage> SlippagePtr;
 ...
 */
typedef std::auto_ptr<Commission> CommissionPtr;
...
typedef unsigned __int64 PositionId;
...
class CORE_API PositionAbstr {
 public:
  ...
  virtual const ManagedPtr<const std::string> getUserString() const = 0;
  ...
  virtual size_t getCloseBar() const
      throw(PositionCloseOperationOnOpenPositionException) = 0;
  /**
  ...
  virtual const std::string& getCloseName() const
      throw(PositionCloseOperationOnOpenPositionException) = 0;
  /**
   * Does a complete dump of the position
   *
   * Will send a string representation of the position to the output stream
   *
   * <!-- \htmlonly <FONT COLOR="RED">TODO: give an example, show what fields
   * are dumped </FONT> \endhtmlonly -->
   *
   * <B>Not thread safe</B>
   *
   * @param os     Output stream to which the contents of the position is to be
   * dumped to
   * @return A reference to the output stream
   */
  virtual std::ostream& dump(std::ostream& os) const = 0;
  /**
   * Does a dump of the position contents, but with less information than the
   * full dump
   *
   * Will send a string representation of major fields of a position to the
   * output stream
   *
   * <!-- \htmlonly <FONT COLOR="RED">TODO: give an example, show what fields
   * are dumped </FONT> \endhtmlonly -->
   ...
   * @return A reference to the output stream
   */
  virtual std::ostream& dumpMin(std::ostream& os) const = 0;
};

typedef ManagedPtr<tradery::PositionAbstr> PositionAbstrPtr;
...
class Position : public PositionAbstr {
 ...
 private:
  bool isValid() const { return _pos; }
  bool validate() const {
    if (isValid())
      return true;
    else
 ...
 public:
  ...
  const ManagedPtr<const std::string> getUserString() const {
    validate();
...
class CORE_API Signal {
 ...
 public:
   ...
   */
  static const char* signalTypeAsString(SignalType type);

 public:
  virtual ~Signal() {}

 public:
  virtual bool applySignalSizing() const = 0;
  /**
   * Creates a duplicate copy of the current signal object
   *
   * @return Pointer to the new signal object
   */
  virtual const Signal* clone() const = 0;
   ...
   * @return Number of shares
   */
  virtual unsigned int shares() const = 0;
  /**
   ...
   */
  virtual const tradery::Position position() const = 0;
  /**
  ...
  virtual const ConstStringPtr systemName() const = 0;
  virtual bool isShort() const = 0;
  virtual bool isLong() const = 0;
  virtual bool isEntryPosition() const = 0;
  virtual bool isExitPosition() const = 0;
  virtual bool hasPrice() const = 0;
  /**
  ...
  virtual void setShares(unsigned int shares) = 0;

  virtual void disable() = 0;
  virtual bool isEnabled() const = 0;
  virtual bool isDisabled() const = 0;
  virtual const ConstStringPtr systemId() const = 0;
};

typedef ManagedPtr<Signal> SignalPtr;
...
class SignalHandler : public PluginConfiguration {
 public:
   ...
   */
  virtual void signal(SignalPtr _signal) throw(SignalHandlerException) = 0;
};
...
class PositionHandler {
 public:
   ...
   */
  virtual void onPositionConst(const Position pos) {}
};
 ...
 * @see PositionsContainer::forEach
 */
class PositionEqualPredHandler : public PositionEqualPredicate,
...
class PositionsContainer {
 public:
   ...
   */
  CORE_API static PositionsContainer* create();
  CORE_API static PositionsContainer* create(const PositionsContainer* pc);
   ...
   */
  virtual void forEach(PositionHandler& onPosition,
                       const PositionEqualPredicate& pred) = 0;
  /**
   ...
   * @see PositionEqualPredicate
   */
  virtual void forEachNot(PositionHandler& onPosition,
                          const PositionEqualPredicate& pred) = 0;
  /**
   * Loops through all positions in the container that meet the criteria defined
  ...
  virtual void forEachOr(PositionHandler& positionHandler,
                         std::vector<PositionEqualPredicate*> predicates) = 0;
  /**
  ...
  virtual void forEachConst(PositionHandler& onPosition,
                            const PositionEqualPredicate& pred) const = 0;
  /**
  ...
  virtual void forEachOpenPosition(OpenPositionHandler& openPositionHandler,
                                   Bars bars, size_t bar) = 0;
  virtual void forEachOpenPosition(
      OpenPositionHandler1& openPositionHandler) = 0;
   ...
   */
  virtual void sort(PositionLessPredicate& predicate,
  ...
  /**
   * Sends a string representation of the container contents to an output stream
   *
   * This method dumps the complete info about each position
   * <!-- \htmlonly <FONT COLOR="RED">TODO: document what the dump looks like,
   * what fields are dumped etc</FONT> \endhtmlonly -->
   *
   * @param os     The output stream to dump to
   * @return A const reference to the output stream
   */
  virtual std::ostream& dump(std::ostream& os) const = 0;
  /**
   * Sends a limited string representation of the cotainer contents to an output
   * stream
   *
   * This method only considers the important fields of each positions
   *
   * <!-- \htmlonly <FONT COLOR="RED">TODO: document what the limited dump looks
   * like, what fields are dumped etc</FONT> \endhtmlonly -->
   *
   * @param os     The output stream to dump to
   *
   * @return A const reference to the output stream
...
class PositionsPtrVector
    : public std::vector<const tradery::PositionsContainer*> {
 private:
  Mutex _m;

 public:
  void add(const PositionsContainer* pc) {
    Lock lock(_m);
...
class PositionsIteratorAbstr {
 public:
  static PositionsIteratorAbstr* create(const PositionsContainer* pc);
...
class PositionsIteratorConst {
 private:
  ManagedPtr<PositionsIteratorAbstr> _pi;

 public:
  PositionsIteratorConst(const PositionsContainer* pc)
...
class PositionsIterator {
 ...
 public:
  PositionsIterator(const PositionsContainer* pc)
      : _pi(PositionsIteratorAbstr::create(pc)) {}
...
typedef std::vector<ManagedPtr<PositionsContainer> > PositionsContainerVector;

class PositionsVector : public PositionsContainerVector {
 private:
  ManagedPtr<PositionsContainer> _all;
  mutable Mutex _mx;

 public:
  PositionsVector() : _all(PositionsContainer::create()) {}

  PositionsContainer* getNewPositionsContainer() {
    Lock lock(_mx);
    PositionsContainer* p = PositionsContainer::create();
    push_back(p);
    return p;
  }
  ...
  PositionsContainer* populateAllPositions() {
    ...
    for (size_t n = 0; n < __super::size(); n++)
      _all->nonDestructiveAppend(__super::at(n).get());
  ...
  size_t count() const {
    ...
    for (const_iterator i = __super::begin(); i != __super::end(); i++)
      size += (*i)->count();
    return size;
  }
...
#define TAB "\t"
#define TD_OPEN "<td class=\"c\">"
#define TD_OPEN_NOWRAP "<td nowrap class=\"c\">"
#define TD_CLOSE "</td>"
...
class PositionToHTMLFormat : public PositionFormatBase {
 ...
 public:
  ...
  PositionToHTMLFormat(std::ostream& os, std::ostream& desc,
                       size_t linesPerPage = 25, bool dateOnly = true)
  ...
  virtual void header() const {
    ...
    _desc << "header="
          << "<tr class=\"h\"> <td class=\"h\">Long/ Short</td> <td "
             "class=\"h\">Symbol</td> <td class=\"h\">Shares</td> <td "
             "class=\"h\">Entry "
          << dtTitle
          << "</td> <td class=\"h\">Entry price</td> <td class=\"h\">Entry "
             "name</td> <td class=\"h\">Exit "
          << dtTitle
          << "</td> <td class=\"h\">Exit price</td> <td class=\"h\">Exit "
             "name</td> <td class=\"h\">Gain</td> <td class=\"h\">Gain %</td> "
             "<td class=\"h\">System</td></tr>"
          << std::endl;
  }
  virtual void line(const Position pos) const {
    ...
    std::string entryDateTime =
        _dateOnly ? st.date().to_simple_string() : st.to_simple_string();
    std::string closeDateTime;
    if (!pos.isOpen()) {
      DateTime et = pos.getCloseTime();
      closeDateTime =
          _dateOnly ? et.date().to_simple_string() : et.to_simple_string();
    } else
      closeDateTime = "---";
    ...
    std::string title =
        pos.isLong() ? "alt='Long' title='Long'" : "alt='Short' title='Short'";
    ...
    _os << "<td class=\"c" << (pos.isLong() ? " lg" : " sh") << "\">"
        << TD_CLOSE;
    ...
    _os << TD_OPEN << std::fixed << std::setprecision(2) << pos.getEntryPrice()
        << TD_CLOSE;
    ...
    if (pos.isOpen())
      ...
      _os << "<td nowrap class=\"" << c << "\">" << std::fixed
          << std::setprecision(2) << pos.getGain() << TD_CLOSE;
      _os << "<td nowrap class=\"" << c << "\">" << std::fixed
          << std::setprecision(2) << pos.getPctGain() << "%" << TD_CLOSE;
    }

    assert(pos.getUserString());
...
class PositionsContainerToFormat {
 public:
  PositionsContainerToFormat(const PositionsContainer& pc,
                             PositionFormatBase& pf,
                             unsigned __int64 maxLines = 0) {
    pc.forEachConst(pf);
    if (!pf.empty()) pf.footer();
  }

  PositionsContainerToFormat(const PositionsVector& pcs, PositionFormatBase& pf,
                             unsigned __int64 maxLines = 0) {
    for (PositionsVector::const_iterator i = pcs.begin(); i != pcs.end(); i++)
      (*i)->forEachConst(pf);
    if (!pf.empty()) pf.footer();
  }
};
...
class PositionsContainerToHTML : public PositionsContainerToFormat {
 public:
  // lines to generate
  // all lines if 0
  PositionsContainerToHTML(const PositionsContainer& pc, std::ostream& os,
                           std::ostream& desc, size_t linesPerPage)
      : PositionsContainerToFormat(
            pc, PositionToHTMLFormat(os, desc, linesPerPage)) {}

  // lines to generate
  // all lines if 0
  PositionsContainerToHTML(const PositionsVector& pcs, std::ostream& os,
                           std::ostream& desc, size_t linesPerPage)
      : PositionsContainerToFormat(
            pcs, PositionToHTMLFormat(os, desc, linesPerPage)) {}
};
...
class PositionIdNotFoundException : public SystemException {
 ...
 public:
  ...
  const std::string message() const {
    ...
    os << "Could not find position with id " << _id;
...
class PositionsI {
 public:
  ...
  virtual void applyTimeBased(Bars bars, size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyTimeBasedAtMarket(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyTimeBasedAtClose(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyStopLoss(Bars bars, size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyTrailing(Bars, size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyBreakEvenStop(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyReverseBreakEvenStop(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyProfitTarget(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyProfitTargetLong(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyProfitTargetShort(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyAutoStops(Bars bars, size_t barIndex) throw(
      BarIndexOutOfRangeException) = 0;
  virtual PositionId buyAtMarket(
      Bars bars, size_t barIndex, size_t shares,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
  virtual PositionId buyAtClose(
      Bars bars, size_t barIndex, size_t shares,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
  virtual PositionId buyAtStop(
      Bars bars, size_t barIndex, double stopPrice, size_t shares,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
  virtual PositionId buyAtLimit(
      Bars bars, size_t barIndex, double limitPrice, size_t shares,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
  virtual bool sellAtMarket(
      Bars bars, size_t barIndex, Position pos,
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     SellingShortPositionException,
                                     ClosingAlreadyClosedPositionException) = 0;
  virtual bool sellAtMarket(
      Bars bars, size_t barIndex, PositionId pos,
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     SellingShortPositionException,
                                     ClosingAlreadyClosedPositionException,
                                     PositionIdNotFoundException) = 0;
  ...
  virtual bool sellAtStop(
      Bars bars, size_t barIndex, Position pos, double stopPrice,
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     SellingShortPositionException,
                                     ClosingAlreadyClosedPositionException) = 0;
  virtual bool sellAtStop(
  ...
  virtual bool sellAtLimit(
      Bars bars, size_t barIndex, Position pos, double limitPrice,
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     SellingShortPositionException,
                                     ClosingAlreadyClosedPositionException) = 0;
  virtual bool sellAtLimit(
      ...
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     ...
                                     ClosingAlreadyClosedPositionException,
                                     PositionIdNotFoundException) = 0;
  virtual PositionId shortAtMarket(
      Bars bars, size_t barIndex, size_t shares,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
  virtual PositionId shortAtClose(
      Bars bars, size_t barIndex, size_t shares,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
  virtual PositionId shortAtStop(
      Bars bars, size_t barIndex, double stopPrice, size_t shares,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
  virtual PositionId shortAtLimit(
      Bars bars, size_t barIndex, double limitPrice, size_t shares,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
  ...
  virtual void closeAllLongAtStop(
      Bars bars, size_t barIndex, double price,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyTimeBased(size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyStopLoss(size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyTrailing(size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyBreakEvenStop(size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyBreakEvenStopLong(size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyBreakEvenStopShort(size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyReverseBreakEvenStop(size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyReverseBreakEvenStopLong(
      size_t barIndex, Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyReverseBreakEvenStopShort(
      size_t barIndex, Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyProfitTarget(size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyProfitTargetLong(size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyProfitTargetShort(size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyAutoStops(size_t barIndex) throw(
      BarIndexOutOfRangeException) = 0;
  virtual PositionId buyAtMarket(
      size_t barIndex, size_t shares, const std::string& name,
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) = 0;
  virtual PositionId buyAtClose(
      size_t barIndex, size_t shares, const std::string& name,
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) = 0;
  virtual PositionId buyAtStop(
      size_t barIndex, double stopPrice, size_t shares, const std::string& name,
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) = 0;
  virtual PositionId buyAtLimit(
      size_t barIndex, double limitPrice, size_t shares,
      const std::string& name,
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) = 0;
  virtual bool sellAtMarket(
      size_t barIndex, Position pos,
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     SellingShortPositionException,
                                     ClosingAlreadyClosedPositionException) = 0;
  virtual bool sellAtMarket(
      size_t barIndex, PositionId pos,
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     SellingShortPositionException,
                                     ClosingAlreadyClosedPositionException,
                                     PositionIdNotFoundException) = 0;
  ...
  virtual bool sellAtClose(
      ...
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     ...
                                     PositionIdNotFoundException) = 0;
  virtual bool sellAtStop(
      size_t barIndex, Position pos, double stopPrice,
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     SellingShortPositionException,
                                     ClosingAlreadyClosedPositionException) = 0;
  virtual bool sellAtStop(
      size_t barIndex, PositionId pos, double stopPrice,
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     SellingShortPositionException,
                                     ClosingAlreadyClosedPositionException,
                                     PositionIdNotFoundException) = 0;
  virtual bool sellAtLimit(
      size_t barIndex, Position pos, double limitPrice,
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     SellingShortPositionException,
                                     ClosingAlreadyClosedPositionException) = 0;
  virtual bool sellAtLimit(
      size_t barIndex, PositionId pos, double limitPrice,
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     SellingShortPositionException,
                                     ClosingAlreadyClosedPositionException,
                                     PositionIdNotFoundException) = 0;
  ...
  virtual bool coverAtLimit(
      ...
      const std::string& name) throw(BarIndexOutOfRangeException,
                                     ...
                                     ClosingAlreadyClosedPositionException,
                                     PositionIdNotFoundException) = 0;
  virtual void closeAllAtMarket(size_t barIndex, const std::string& name) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void closeAllShortAtMarket(
      size_t barIndex,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
  virtual void closeAllLongAtMarket(
      size_t barIndex,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
  virtual void closeAllAtClose(size_t barIndex, const std::string& name) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void closeAllShortAtClose(
      size_t barIndex,
      const std::string& name) throw(BarIndexOutOfRangeException) = 0;
...
class OpenPositionsIterator : public OpenPositionsIteratorAbstr {
 private:
  tradery::ManagedPtr<OpenPositionsIteratorAbstr> _it;

 public:
  OpenPositionsIterator() {}
  OpenPositionsIterator(ManagedPtr<OpenPositionsIteratorAbstr> it) : _it(it) {}
 ...
 * @see PositionsContainer
 */
typedef ManagedPtr<PositionsContainer> PositionsContainerPtr;
...
class PositionsManagerAbstr {
 ...
 private:
  virtual PositionId buyAtPrice(
      ...
      PositionId id) throw(BarIndexOutOfRangeException) = 0;
 ...
 public:
  ...
  virtual void setSystemName(const ConstStringPtr str) = 0;
  virtual const ConstStringPtr systemName() const = 0;

  virtual void setSystemId(const std::string& str) = 0;
  virtual void setSystemId(const ConstStringPtr str) = 0;
  virtual const ConstStringPtr systemId() const = 0;
   ...
   * @see Commission
   */
  CORE_API static PositionsManagerAbstr* create(PositionsContainer* posList,
                                                DateTime startTrades,
                                                DateTime endTrades,
                                                Slippage* slippage = 0,
  ...
  */
  ...
  virtual void applyStopLoss(Bars bars, size_t barIndex, Position pos) throw(
      BarIndexOutOfRangeException) = 0;
  virtual void applyStopLossLong(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyStopLossShort(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  /**
  ...
  virtual void applyBreakEvenStop(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyBreakEvenStopLong(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  virtual void applyBreakEvenStopShort(
      Bars bars, size_t barIndex,
      Position pos) throw(BarIndexOutOfRangeException) = 0;
  /**
   ...
   */
  virtual PositionId buyAtMarket(
      Bars bars, size_t barIndex, size_t shares, const std::string& name,
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) = 0;
  /**
  ...
  //@{
  /**
   * dumps the contents of the PositionsManager object in readable form to the
   * output stream
   *
   * <!-- \htmlonly <FONT COLOR="RED">TODO: thread safe?</FONT> \endhtmlonly -->
  ...
  /**
   * dumps the contents of the PositionsManager object in readable form to the
   * output stream. It only shows the most important fields.
   *
   * <!-- \htmlonly <FONT COLOR="RED">TODO: thread safe?</FONT> \endhtmlonly -->
   *
   * @param os     output stream
...
typedef ManagedPtr<PositionsManagerAbstr> PositionsManagerAbstrPtr;
...
class Positions : public PositionsManagerAbstr {
 ...
 public:
  ...
  virtual void setSystemName(const ConstStringPtr str) {
    validate();
    _positions->setSystemName(str);
  }
  virtual const ConstStringPtr systemName() const {
    validate();
  ...
  virtual void setSystemId(const ConstStringPtr str) {
    validate();
    _positions->setSystemId(str);
  }
  virtual const ConstStringPtr systemId() const {
    validate();
  ...
  virtual PositionId buyAtPrice(
    ...
    validate();
    return _positions->buyAtPrice(bars, barIndex, price, shares, name,
  ...
  virtual PositionId shortAtPrice(
      ...
      PositionId id) throw(BarIndexOutOfRangeException) {
    validate();
    return _positions->shortAtPrice(bars, barIndex, price, shares, name,
                                    applyPositionSizing, id);
  }
  virtual Position getPosition(PositionId id) {
    validate();
  ...
  virtual size_t positionCount() const {
    validate();
  ...
  virtual void registerSignalHandler(SignalHandler* signalHandler) {
    validate();
  ...
  virtual void installTimeBasedExit(Index bars) {
    validate();
    _positions->installTimeBasedExit(bars);
  }
  virtual void installTimeBasedExitAtMarket(Index bars) {
    validate();
    _positions->installTimeBasedExitAtMarket(bars);
  }
  virtual void installTimeBasedExitAtClose(Index bars) {
    validate();
    _positions->installTimeBasedExitAtClose(bars);
  }
  virtual void installBreakEvenStop(double level) {
    validate();
    return _positions->installBreakEvenStop(level);
  }
  virtual void installBreakEvenStopLong(double level) {
    validate();
    return _positions->installBreakEvenStopLong(level);
  }
  virtual void installBreakEvenStopShort(double level) {
    validate();
    return _positions->installBreakEvenStopShort(level);
  }
  virtual void installReverseBreakEvenStop(double level) {
    validate();
    _positions->installReverseBreakEvenStop(level);
  }
  virtual void installReverseBreakEvenStopLong(double level) {
    validate();
    _positions->installReverseBreakEvenStopLong(level);
  }
  virtual void installReverseBreakEvenStopShort(double level) {
    validate();
    _positions->installReverseBreakEvenStopShort(level);
  }
  virtual void installStopLoss(double level) {
    validate();
    _positions->installStopLoss(level);
  }
  virtual void installStopLossLong(double level) {
    validate();
    _positions->installStopLossLong(level);
  }
  virtual void installStopLossShort(double level) {
    validate();
    _positions->installStopLossShort(level);
  }
  virtual void installProfitTarget(double level) {
    validate();
    _positions->installProfitTarget(level);
  }
  virtual void installProfitTargetShort(double level) {
    validate();
    _positions->installProfitTargetShort(level);
  }
  virtual void installProfitTargetLong(double level) {
    validate();
    _positions->installProfitTargetLong(level);
  }
  virtual void installTrailingStop(double trigger, double lossLevel) {
    validate();
    _positions->installTrailingStop(trigger, lossLevel);
  }
  virtual void applyTimeBased(Bars bars, size_t barIndex,
                              Position pos) throw(BarIndexOutOfRangeException) {
    validate();
  ...
  virtual void applyStopLoss(Bars bars, size_t barIndex,
                             Position pos) throw(BarIndexOutOfRangeException) {
    validate();
  ...
  virtual void applyTrailing(Bars bars, size_t barIndex,
                             Position pos) throw(BarIndexOutOfRangeException) {
    validate();
  ...
  virtual PositionId buyAtMarket(
    ...
    validate();
    return _positions->buyAtMarket(bars, barIndex, shares, name,
                                   applyPositionSizing);
  }
  virtual PositionId buyAtClose(
      Bars bars, size_t barIndex, size_t shares, const std::string& name,
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) {
    validate();
    return _positions->buyAtClose(bars, barIndex, shares, name,
                                  applyPositionSizing);
  }
  virtual PositionId buyAtStop(
      Bars bars, size_t barIndex, double stopPrice, size_t shares,
      ...
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) {
    validate();
    return _positions->buyAtStop(bars, barIndex, stopPrice, shares, name,
                                 applyPositionSizing);
  }
  virtual PositionId buyAtLimit(
      Bars bars, size_t barIndex, double limitPrice, size_t shares,
      ...
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) {
    validate();
    return _positions->buyAtLimit(bars, barIndex, limitPrice, shares, name,
                                  applyPositionSizing);
  }
  virtual bool sellAtMarket(
      Bars bars, size_t barIndex, Position pos,
  ...
  virtual PositionId shortAtMarket(
      ...
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) {
    validate();
    return _positions->shortAtMarket(bars, barIndex, shares, name,
                                     applyPositionSizing);
  }
  virtual PositionId shortAtClose(
      Bars bars, size_t barIndex, size_t shares, const std::string& name,
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) {
    validate();
    return _positions->shortAtClose(bars, barIndex, shares, name,
                                    applyPositionSizing);
  }
  virtual PositionId shortAtStop(
      Bars bars, size_t barIndex, double stopPrice, size_t shares,
      ...
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) {
    validate();
    return _positions->shortAtStop(bars, barIndex, stopPrice, shares, name,
                                   applyPositionSizing);
  }
  virtual PositionId shortAtLimit(
      Bars bars, size_t barIndex, double limitPrice, size_t shares,
      ...
      bool applyPositionSizing = true) throw(BarIndexOutOfRangeException) {
    validate();
    return _positions->shortAtLimit(bars, barIndex, limitPrice, shares, name,
                                    applyPositionSizing);
  }
  virtual bool coverAtMarket(
      Bars bars, size_t barIndex, Position pos,
  ...
  virtual void closeAllShortAtLimit(
      Bars bars, size_t barIndex, double limitPrice,
      const std::string& name) throw(BarIndexOutOfRangeException) {
    validate();
  ...
  virtual std::ostream& dump(std::ostream& os) const {
    validate();
    return _positions->dump(os);
  }
  virtual std::ostream& dumpMin(std::ostream& os) const {
    validate();
    return _positions->dumpMin(os);
  }
};
...
class Parameters : public double_vector {
 public:
  ...
  void setValue(size_t index, double value) throw(ParametersException) {
    try {
      at(index) = value;
...
class Runnable : public PluginConfiguration {
 ...
 public:
  Runnable(const Info& info, const std::string& userString)
      : PluginConfiguration(info),
        _parameters(0),
        _userString(new std::string(userString)) {}

  virtual ~Runnable() {}

  const ConstStringPtr getUserString() const { return _userString; }
   ...
   * @see PositionsManager
   */
  virtual void init(const DataCollection* bars,
                    PositionsManagerAbstr* positions,
                    chart::Chart* chartHandler,
                    const tradery::ExplicitTrades* triggers) = 0;
  /**
...
/**
 * Defines the position sizing strategie defined in the current session.
 *
 * Is a component of the RuntimeParams class
 *
 * @see RuntimeParams
 * @see SessionInfo
 */
class CORE_API PositionSizing {
 public:
  // indicates the maximum number of open positions held at any one time

 private:
  const PositionSizingParams& _psp;
  PositionsContainer& _pc;

 public:
  PositionSizing(PositionsContainer& pc, const PositionSizingParams& psp);
  virtual ~PositionSizing() {}
