...
class EquityDateRange : public DateRange {
 public:
  EquityDateRange(const SessionInfo& si) {
    ...
    tradery::SymbolsIterator* it(si.symbolsIterator());
    assert(it != 0);

    for (SymbolConstPtr symbol = it->getFirst(); symbol.get() != 0;
         symbol = it->getNext()) {
      ...
      if (bars && bars.size() > 0) {
        ...
        if (_from.is_neg_infinity())
          ...
          _from = std::min(_from, bars.time(0).date());
        ...
        if (_to.is_pos_infinity())
          ...
          _to = std::max(_to, bars.time(bars.size() - 1).date());
      }
    }
    ...
    if (_from.is_neg_infinity()) _from = Date("01/01/1960");
    if (_to.is_pos_infinity()) _to = LocalTimeSec().date();
  }
};
...
class StatsToFormat {
 ...
 public:
  ...
  virtual void row(const std::string& name, double all, double longs,
                   double shorts, double bh, bool minMax = true,
                   bool pct = false, unsigned int precision = 2) const = 0;
  virtual void row(const std::string& name, const Date& all, const Date& longs,
                   const Date& shorts, const Date& bh) const = 0;
};
...
class StatsToHTML : public StatsToFormat {
 ...
 public:
  ...
  virtual void subtitle(const std::string& st) const {
    _os << "<tr class=\"subheader\"><td colspan=\"5\">" << st << "</td></tr>"
        << std::endl
        << "<tr>" << std::endl;
    _count = 0;
  }

  virtual void header(const DateRange& range) const {
    _os << "<table class=\"statsTable\">" << std::endl;
    _os << "<tr class=\"h\"> <td class=\"h\"></td><td class=\"h\">Total "
           "stats</td> <td class=\"h\">Long stats</td> <td class=\"h\">Short "
           "stats</td> <td class=\"h\">Buy & Hold stats</td> </tr>"
        << std::endl;
    _os << "<tr class=\"d1\"><td class=\"c\">Range</td><td class=\"c\" "
           "colspan=\"4\" align=\"center\">"
        << range.from().to_simple_string() << " - "
        << range.to().to_simple_string() << "</td></tr>";
    _count = 0;
  }

  virtual void footer() const { _os << "</table>" << std::endl; }

  static std::string getColor(double value, double maxVal, double minVal,
                              bool maxMin) {
    std::string style =
        value >= 0 ? "style=\"color:blue;" : "style=\"color:red;";
  ...
  virtual void row(const std::string& name, double all, double longs,
                   double shorts, double bh, bool maxMin = true,
                   bool pct = false, unsigned int precision = 2) const {
    ...
    _os << "\t<td class=\"c\" " << getColor(all, maxVal, minVal, maxMin)
        << std::fixed << std::setprecision(precision) << all
        << (pct ? " %" : "") << "</td>" << std::endl;
    _os << "\t<td class=\"c\" " << getColor(longs, maxVal, minVal, maxMin)
        << std::fixed << std::setprecision(precision) << longs
        << (pct ? " %" : "") << "</td>" << std::endl;
    _os << "\t<td class=\"c\" " << getColor(shorts, maxVal, minVal, maxMin)
        << std::fixed << std::setprecision(precision) << shorts
        << (pct ? " %" : "") << "</td>" << std::endl;
    _os << "\t<td class=\"c\" " << getColor(bh, maxVal, minVal, maxMin)
  ...
  virtual void row(const std::string& name, const Date& all, const Date& longs,
                   const Date& shorts, const Date& bh) const {
    _os << "<tr class=\"" << (_count % 2 ? "d0\"" : "d1\"") << ">" << std::endl;
    _os << "\t<td class=\"c\">" << name << "</td>" << std::endl;
    _os << "\t<td class=\"c\">"
        << (all.is_not_a_date() ? "" : all.to_simple_string()) << "</td>"
        << std::endl;
    _os << "\t<td class=\"c\">"
        << (longs.is_not_a_date() ? "" : longs.to_simple_string()) << "</td>"
        << std::endl;
    _os << "\t<td class=\"c\">"
        << (shorts.is_not_a_date() ? "" : shorts.to_simple_string()) << "</td>"
        << std::endl;
    _os << "\t<td class=\"c\">"
        << (bh.is_not_a_date() ? "" : bh.to_simple_string()) << "</td>"
        << std::endl;
    _os << "</tr>" << std::endl;
    _count++;
  }
...
class StatsToCSV : public StatsToFormat {
 public:
  ...
  virtual void header(const DateRange& dateRange) const {
    _os << ",Total stats,Long stats,Short stats,Buy & Hold stats" << std::endl;
    _os << "Date Range," << dateRange.from().to_simple_string() << ","
        << dateRange.to().to_simple_string() << std::endl;
  }
...
class CurrentPriceSource {
 public:
  ...
  virtual double get(const std::string& symbol) const
      throw(DataNotAvailableForSymbolException) = 0;
};
...
class SlippageStats : public PositionHandler {
 ...
 public:
  ...
  SlippageStats operator+(const SlippageStats& slippageStats) const {
    ...
    temp._totalCloseSlippage =
        _totalCloseSlippage + slippageStats._totalCloseSlippage;
    temp._totalOpenSlippage =
        _totalOpenSlippage + slippageStats._totalOpenSlippage;
...
class CommissionStats : public PositionHandler {
 ...
 public:
  ...
  CommissionStats operator+(const CommissionStats& commissionStats) const {
    ...
    temp._totalCloseCommission =
        _totalCloseCommission + commissionStats._totalCloseCommission;
    temp._totalOpenCommission =
        _totalOpenCommission + commissionStats._totalOpenCommission;
...
class PosStats {
 ...
 public:
  ...
  double pctGainLoss() const {
    ...
    LOG(log_info, "In pctGainLoss, initialcap: " << _initialCapital
                                                 << ", gain: " << _gain);
  ...
  double expectancy() const {
    LOG(log_info,
        "total loss: " << _totalLoss << ", total gain: " << _totalGain);
    LOG(log_info,
        "pct winning: " << pctWinning()
                        << ", avg gain per pos: " << averageGainPerWinningPos()
                        << ", pct losing: " << pctLosing()
                        << ", avg loss per pos: " << averageLossPerLosingPos());
  ...
  PosStats operator+(const PosStats& posStats) const {
    ...
    temp._maxGainPerPos = std::max(_maxGainPerPos, posStats.maxGainPerPos());
    temp._maxLossPerPos = std::min(_maxLossPerPos, posStats.maxLossPerPos());
    temp._maxPctGainPerPos =
        std::max(_maxPctGainPerPos, posStats.maxPctGainPerPos());
    temp._maxPctLossPerPos =
        std::min(_maxPctLossPerPos, posStats.maxPctLossPerPos());
  ...
  void calculateAnnualizedPctGain(const DateRange& dr) {
    ...
    _annualizedPctGain =
        dr.duration() > 0
            ? (pow(_endingCapital / _initialCapital, 1.0 / dr.duration()) - 1) *
                  100.0
            : 0;
...
class CORE_API Stats {
 ...
 public:
  ...
  double pctWinningClosed() const {
    return _allPosStats.count() > 0 ? (double)_closedPosStats.winningCount() /
                                          (double)_allPosStats.count() * 100.0
                                    : 0;
  }
  double pctLosingClosed() const {
    return _allPosStats.count() > 0 ? (double)_closedPosStats.losingCount() /
                                          (double)_allPosStats.count() * 100.0
                                    : 0;
  }
  double pctNeutralClosed() const {
    return _allPosStats.count() > 0 ? (double)_closedPosStats.neutralCount() /
                                          (double)_allPosStats.count() * 100.0
                                    : 0;
  }

  double pctWinningOpen() const {
    return _allPosStats.count() > 0 ? (double)_openPosStats.winningCount() /
                                          (double)_allPosStats.count() * 100.0
                                    : 0;
  }
  double pctLosingOpen() const {
    return _allPosStats.count() > 0 ? (double)_openPosStats.losingCount() /
                                          (double)_allPosStats.count() * 100.0
                                    : 0;
  }
  double pctNeutralOpen() const {
    return _allPosStats.count() > 0 ? (double)_openPosStats.neutralCount() /
                                          (double)_allPosStats.count() * 100.0
                                    : 0;
  }
  ...
  void calculateScore(double pctExposure, double ulcerIndex) {
    ...
    _traderyScore = apg * (1 - sign * pctExposure / 100) *
                    (1.00 - sign * std::min(ulcerIndex, 20.0) / 20.0);

    LOG(log_info, "score: " << _traderyScore << ", APG: " << apg
                            << ", exposure: " << pctExposure
...
class CORE_API StatsCalculator : public Stats, public PositionHandler {
 ...
 public:
   ...
   * @see PositionsContainer
   */
  void calculate(PositionsContainer& positions,
                 const PositionEqualPredicate& predicate);
  /**
 ...
 public:
  ...
  bool operator<(ProcessPosition& pos) const {
    ...
    if (time() < pos.time())
      ...
      if (crtType == market_order || otherType == close_order) return true;
...
class OpenPosition : public ProcessPosition {
 public:
  ...
  virtual bool lower(ProcessPosition& pos) const { return id() == pos.id(); }
  virtual bool entry() const { return true; }
};
...
class ClosePosition : public ProcessPosition {
 public:
  ...
  virtual bool entry() const { return false; }
};

class ProcessPositionPtr : public ManagedPtr<ProcessPosition> {
 public:
  ProcessPositionPtr(ProcessPosition* p) : ManagedPtr<ProcessPosition>(p) {}

  bool operator<(ProcessPositionPtr p) const { return **this < *p; }
};

typedef public std::multiset<ProcessPositionPtr> ProcessPositionsBase;
class ProcessPositions : public ProcessPositionsBase {
 ...
 public:
  ...
  void dump(std::ostream& os) const {
    for (const ProcessPosition* p = getFirst(); p != 0; p = getNext())
      p->dump(os);
  }
};

typedef tradery::ManagedPtr<ProcessPositions> ProcessPositionsPtr;

class DateToProcessPositions : public tradery::PositionHandler {
 private:
  typedef std::map<Date, ProcessPositionsPtr> DPPMap;
 ...
 private:
  void insertPosition(Position pos) {
    ...
    if (i == _map.end())
      i = _map.insert(DPPMap::value_type(
                          pos.getEntryDate(),
                          ProcessPositionsPtr(new ProcessPositions())))
              .first;
 ...
 public:
  ...
  virtual void onPositionConst(const Position pos) { insertPosition(pos); }
  ...
  void dump(std::ostream& os) const {
    os << "****** all positions to process *******" << std::endl;
    for (DPPMap::const_iterator i = _map.begin(); i != _map.end(); i++) {
      const ProcessPositionsPtr pp = i->second;

      os << i->first.to_simple_string() << std::endl;
      pp->dump(os);
    }
    os << "***************************************" << std::endl;
  }
...
class Equity {
 ...
 public:
  ...
  void dump(std::ostream& os) {
    //      os << _T( "total: " ) << _total << _T( ", long: " ) << _lg << _T( ",
    //      short: " ) << _sh << _T( ", cash: " ) << _cash << std::endl;
  }
...
typedef std::map<Date, Equity> EquityCurveBase;
...
class EquityCurve : public EquityCurveBase {
 private:
  ...
  ManagedPtr<const PositionEqualPredicate> _pred;
  ...
  //      DebugBreak();
    ...
    for (Date d = _edr.from(); d <= _edr.to(); d = d + Days(1)) {
      ...
      for (ProcessPosition* pp = _dpp.getFirst(d); pp != 0;
           pp = _dpp.getNext(d)) {
        Position pos = pp->get();
        assert(pos);
        ...
        if (pp->entry()) {
          ...
          if (_doPosSizing && pos.applyPositionSizing()) {
            if (!posSizing(pos, bars, ec)) continue;
          }
      ...
      //        std::cout << d.to_simple_string() << _T( " - " );
    }
  }
  ...
  bool posSizing(Position pos, const BarsAbstr* bars, const Equity& ec) {
    ...
    if (!ps.maxOpenPos().unlimited() &&
        _openPosCount >= ps.maxOpenPos().get()) {
      pos.disable();
      return false;
    }
    ...
    switch (ps.posSizeType()) {
      ...
      case PositionSizingParams::PosSizeType::pctEquity:
        newShares =
            (size_t)round((ec.getAll().getTotal() * ps.posSize() / 100) /
                          pos.getEntryPrice());
        break;
      case PositionSizingParams::PosSizeType::pctCash:
        newShares = (size_t)round((ec.getAll().getCash() * ps.posSize() / 100) /
                                  pos.getEntryPrice());
        break;
      default:
        assert(false);
    ...
    switch (ps.posSizeLimitType()) {
      ...
      case PositionSizingParams::PosSizeLimitType::pctVolume: {
        ...
        size_t maxShares =
            (size_t)(ps.posSizeLimit() / 100 * bars->volume(bar));

        if (newShares > maxShares) newShares = maxShares;
      } break;
      ...
      default:
        ...
        //              std::cout << _T( "Entry date: " ) <<
        //              pos.getEntryDate().to_simple_string() << _T( ", entry
        //              cost: " ) << pos.getEntryCost( newShares ) << _T( ",
        //              cash: " ) << ec->getCash() << std::endl;
        /*              std::cout << _T( "Entry date: " ) <<
           pos.getEntryDate().to_simple_string() << _T( ", " ); std::cout << _T(
           "Old calc: " ) << pos.getEntryPrice()* newShares << _T( ", " );
              std::cout << _T( "New calc: " ) << pos.getEntryCost( newShares )
           << _T( ", " ); std::cout << _T( "cash: " ) << ec->getCash() <<
           std::endl;
        */

#ifdef EQOUT
    LOG(log_info, "entry cost: " << pos.getEntryCost(
                      newShares) /*_T( ", cash: " ) << ec.getCash() << */);
#endif
 ...
 public:
   ...
   * equity
   */
  EquityCurve(const DateRange& edr, const SessionInfo& si,
              PositionsContainer& pc, bool doPosSizing)
      : _edr(edr),
        _si(si),
        _dpp(pc),
        _openPosCount(0),
 ...
 private:
  const double* getAsArray(std::vector<double>& v,
                           const Eq& (Equity::*f)() const,
                           double (Eq::*g)() const) const {
 ...
 public:
  ...
  double getShortPctExposure() const {
    if (_allSum.getTotal() == 0)
      ...
      return ((_shortSum.getTotal() - _shortSum.getCash()) /
              _allSum.getTotal()) *
             100.0;
  }
  ...
  double getLongPctExposure() const {
    if (_allSum.getTotal() == 0)
      ...
      return ((_longSum.getTotal() - _longSum.getCash()) / _allSum.getTotal()) *
             100.0;
  }
...
class DrawdownCurve {
 ...
 public:
  ...
  DrawdownCurve(const EquityCurve& ec, const Eq& (Equity::*f)() const)
    ...
    for (EquityCurve::const_iterator i = ec.begin(); i != ec.end(); i++) {
  ...
  double ulcerIndex() const {
    return _retracementCount > 0
               ? sqrt(_retracementSqSum / _retracementCount) * 100.0
               : 0;
  }
};
