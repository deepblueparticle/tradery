...
class SessionInfo {
 public:
   ...
   */
  virtual tradery::SymbolsIterator* symbolsIterator() const = 0;
...
class SessionEventHandlerDelegator : public SessionEventHandler {
 ...
 public:
  ...
  virtual void sessionStarted(SessionInfo& so) {
    for (std::set<SessionEventHandler*>::iterator i = _handlers.begin();
         i != _handlers.end(); i++)
      (*i)->sessionStarted(so);
  }
...
class PluginConfiguration : public Info, public SessionEventHandler {
 ...
 public:
   ...
   * @see SymbolsIterator
   */
  tradery::SymbolsIterator* symbolsIterator() const {
    assert(_sessionInfo != 0);
