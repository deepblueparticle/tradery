...
class CORE_API ErrorEvent {
 ...
 public:
   ...
   * @see ErrorEventSink
   */
  ErrorEvent(Types type, Category category, const std::string& systemName,
             const std::string& message, const std::string& symbol);
  ErrorEvent(const ErrorEvent& event)
      : _type(event.type()),
        _systemName(event.systemName()),
  ...
  std::string typeToString() const {
    switch (_type) {
      ...
      case POSITION_CLOSE_OPERATION_ON_OPEN_POSITION_ERROR:
        return "Operation requiring a closed position, but applied to an open "
               "position";
...
class CORE_API ErrorEventSink {
 public:
   ...
   * @see ErrorEvent
   */
  virtual void push(const ErrorEvent& event) = 0;
  /**
   ...
   * sink is empty
   */
  virtual const ErrorEvent* front() const = 0;
  /**
 ...
 */
typedef std::auto_ptr<ErrorEventSink> ErrorEventSinkPtr;
//@}
...
class RunnableRunInfo {
 ...
 public:
   ...
   * @param errors
   */
  RunnableRunInfo(const std::string& status, const std::string& symbol,
                  double dataDuration, double runnableDuration,
                  unsigned __int64 dataUnitCount, bool errors,
                  const std::string& threadName)
      : _status(status),
        _symbol(symbol),
        _dataDuration(dataDuration),
        _runnableDuration(runnableDuration),
        _errors(errors),
        _dataUnitCount(dataUnitCount),
        _threadName(threadName),
        _cpuNumber(getCurrentCPUNumber()) {}
...
class CORE_API Scheduler {
 public:
   ...
   * @see signalHandler
   */
  virtual void addRunnable(
      Runnable* runnable, DataInfoIterator* s, PositionsVector& pos,
      ErrorEventSink* es, SignalHandler* signalHandler = 0,
  ...
  virtual void addRunnable(
      Runnable* runnable, DataInfoIterator* s, PositionsVector& pos,
      ErrorEventSink* es, const std::vector<SignalHandler*>& signalHandlers,
      RunnableRunInfoHandler* runnableRunInfoHandler = 0,
      Slippage* slippage = 0, Commission* commission = 0,
      chart::ChartManager* chartManager = 0,
      const tradery::ExplicitTrades* explicitTrades = 0) = 0;
  ...
  virtual void run(
      ...
      DateTime startTradesDateTime =
          DateTime()) throw(SchedulerReentrantRunCallException) = 0;
  /**
 ...
 * @see Scheduler
 */
typedef std::auto_ptr<Scheduler> SchedulerPtr;
...
class CORE_API Session {
 ...
 public:
   ...
   * @see BarRange
   */
  void run(bool asynch, unsigned int threads, bool cpuAffinity,
           DateTimeRangePtr range, const DateTime startTradesDateTime);
  /**
...
/**
 * Creates a basic error event sink. This can be used to receive and store
 * errors generated during system runs.
 *
 * @return
 */
CORE_API ErrorEventSink* createBasicErrorEventSink();
/**
