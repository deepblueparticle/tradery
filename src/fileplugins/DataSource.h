...
class PosLine {
 ...
 public:
  ...
  std::string toString() const {
    return std::string() << "line: " << _line << ", pos: " << _pos;
  }
};
...
inline bool skipNLInc(std::istream& file) {
  for (std::istream::int_type c = file.peek(); is_nl(c) && !file.eof();
       file.seekg(1, ios_base::cur), c = file.peek())
    ;
#if (defined _DEBUG && defined FILE_DS_DUMP)

  COUT << _T( "pos: " ) << file.tellg() << std::endl;
#endif
...
inline void skipNLDec(std::istream& file) {
  for (std::istream::int_type c = file.peek(); is_nl(c) && file.tellg() > 0;
       file.seekg(-1, ios_base::cur), c = file.peek())
    ;
#if (defined _DEBUG && defined FILE_DS_DUMP)
  COUT << _T( "pos: " ) << file.tellg() << std::endl;
...
inline const PosLine getCrtLine(__int64 pos, std::istream& file) {
  if (pos >= fileSize(file)) return PosLine();

  // set file pointer at pos
  file.seekg(pos);

  if (file.fail()) return PosLine();
...
class PosDateTime {
 ...
 public:
  ...
  std::string toString() const {
    return std::string() << "date: " << _dateTime.date().toString()
                         << ", pos: " << _pos;
  }
...
typedef private std::pair<unsigned __int64, unsigned __int64>
    FilePositionInfoBase;
...
typedef ManagedPtr<FilePositionInfo> FilePositionInfoPtr;
...
class FileDataSource : public tradery::DataSource {
 private:
  ...
  const PosDateTime firstTimeStamp(std::istream& file) const {
    ...
    for (PosLine pos = getCrtLine(0, file); pos;
         pos = getNextLine(pos.pos(), file)) {
  ...
  PosDateTime getCandidate(PosDateTime& begin, PosDateTime& end,
                           std::istream& file) const {
    assert(begin);
    assert(end);
    assert(begin <= end);

    __int64 newPos = (_int64)(begin.posAsDouble() +
                              (end.posAsDouble() - begin.posAsDouble()) / 2.0);
...
#if (defined _DEBUG && defined FILE_DS_DUMP)
    COUT << _T( "prev line: " ) << prev.toString() << std::endl;
#endif
    return prev ? timeStamp(prev.pos(), file) : PosDateTime();
  }

  void trace(const PosDateTime& begin, const PosDateTime& end,
             const PosDateTime& candidate) const {
#if (defined _DEBUG && defined FILE_DS_DUMP)
    COUT << std::endl;
    COUT << _T( "candidate:\t " ) << candidate.toString() << std::endl;
    COUT << _T( "begin:\t " ) << begin.toString() << std::endl;
    COUT << _T( "end:\t " ) << end.toString() << std::endl;
#endif
  }
  ...
  __int64 findStart(const DateTime td, std::istream& file) const {
    ...
    if (!begin) throw DataFileException();
    ...
    if (begin >= td)
      ...
      while (true) {
        ...
        if (candidate == begin) {
#if (defined _DEBUG && defined FILE_DS_DUMP)
          COUT << _T( "candidate == begin, incrementing: " )
               << candidate.toString() << std::endl;
#endif
          candidate = next(candidate, file);

#if (defined _DEBUG && defined FILE_DS_DUMP)
          COUT << _T( "candidate after incrementing: " ) << candidate.toString()
               << std::endl;
#endif
          if (candidate == end) {
            trace(begin, end, candidate);
#if (defined _DEBUG && defined FILE_DS_DUMP)
            COUT << _T( "candidate == end, returning" ) << std::endl;
#endif
            return end.pos();
          }
        } else if (candidate == end) {
#if (defined _DEBUG && defined FILE_DS_DUMP)
          COUT << _T( "candidate == end, decrementing: " )
               << candidate.toString() << std::endl;
#endif
          candidate = prev(candidate, file);
#if (defined _DEBUG && defined FILE_DS_DUMP)
          COUT << _T( "candidate after decrementing: " ) << candidate.toString()
               << std::endl;
#endif
          if (candidate == begin) {
            trace(begin, end, candidate);
#if (defined _DEBUG && defined FILE_DS_DUMP)
            COUT << _T( "candidate == begin, returning" ) << std::endl;
#endif
            return end.pos();
          }
        }
 ...
 private:
   ...
   */
  DataXPtr makeBars(const std::string& symbol, const std::string& ext,
                    DateTimeRangePtr range) const throw(DataSourceException);
  ...
  virtual void fileNotFoundErrorHandler(const std::string& symbol,
    ...
    o << "Could not open data file for symbol \"" << symbol << "\", "
      << fileName;
 ...
 protected:
   ...
   */
  inline FilePositionInfo FileDataSource::parseBars(
      tradery::Addable<Bar>* bars, std::istream& _file, DateTimeRangePtr range,
      const std::string& symbol) const throw(BarException) {
    std::string str;

    __int64 startPos = 0;
    __int64 endPos = fileSize(_file);

    //  COUT << _T( "endpos1: " ) << endPos << std::endl;
    ...
    if (range) {
      ...
      //    COUT << "symbol: " << symbol << ", startDate: " <<
      //    rg->from().date().toString() << _T( "startPos: " ) << std::hex <<
      //    startPos << std::endl;
 ...
 private:
   ...
   * @exception DataSourceException
   */
  virtual const tradery::Bar* parseBarLine(const std::string& str) const
      throw(DataSourceException, BarException) = 0;
 ...
 protected:
  ...
  FileDataSource(
      const Info& info, const std::string& path, const std::string& ext,
      Format format, bool flatData = true,
      ErrorHandlingMode errorHandlingMode = fatal) throw(DataSourceException)
      : DataSource(info),
        _path(path),
        _ext(ext),
        _format(format),
 ...
 public:
  static FileDataSource* make(
      const Info& info, const std::string& path, const std::string& ext,
      Format format, bool flatData = true,
      ErrorHandlingMode errorHandlingMode = fatal) throw(DataSourceException);
  // _extraInfo is extension
  DataXPtr FileDataSource::getData(const DataInfo* dataInfo,
                                   DateTimeRangePtr range) const
  ...
  virtual bool isConsistent(const std::string& stamp, const Symbol& symbol,
                            DateTimeRangePtr range) const
      throw(DataSourceException) {
    return getFileStamp(FileName(_flatData).makePath(
               _path, symbol.symbol(), addExtension(symbol.symbol(), _ext))) ==
           stamp;
  }
...
class FileDataSourceFormat7FieldsBase : public FileDataSource {
 public:
  FileDataSourceFormat7FieldsBase(
      const Info& info, const std::string& path, const std::string& ext,
      Format format, bool flatData,
      ErrorHandlingMode errorHandlingMode) throw(DataSourceException)
      : FileDataSource(info, path, ext, format, flatData, errorHandlingMode) {}

 protected:
  virtual DateTime parseDate(const std::string& date,
                             const std::string& time) const = 0;
  virtual const tradery::Bar* parseBarLine(const std::string& str) const
      throw(DataSourceException, BarException);
};

// format 1 has 7 fields, date, time: m/d/y, h:m:s
class FileDataSourceFormat1 : public FileDataSourceFormat7FieldsBase {
 public:
  FileDataSourceFormat1(
      const Info& info, const std::string& path, const std::string& ext,
      bool flatData,
      ErrorHandlingMode errorHandlingMode) throw(DataSourceException)
      : FileDataSourceFormat7FieldsBase(info, path, ext, format1, flatData,
                                        errorHandlingMode) {}
...
class FileDataSourceFormat2 : public FileDataSourceFormat7FieldsBase {
 public:
  FileDataSourceFormat2(
      const Info& info, const std::string& path, const std::string& ext,
      bool flatData,
      ErrorHandlingMode errorHandlingMode) throw(DataSourceException)
      : FileDataSourceFormat7FieldsBase(info, path, ext, format2, flatData,
                                        errorHandlingMode) {}
...
class FileDataSourceFormat6FieldsBase : public FileDataSource {
 public:
  FileDataSourceFormat6FieldsBase(
      const Info& info, const std::string& path, const std::string& ext,
      Format format, bool flatData,
      ErrorHandlingMode errorHandlingMode) throw(DataSourceException)
      : FileDataSource(info, path, ext, format, flatData, errorHandlingMode) {}
...
class FileDataSourceFormat3 : public FileDataSourceFormat6FieldsBase {
 public:
  FileDataSourceFormat3(
      const Info& info, const std::string& path, const std::string& ext,
      bool flatData,
      ErrorHandlingMode errorHandlingMode) throw(DataSourceException)
      : FileDataSourceFormat6FieldsBase(info, path, ext, format3, flatData,
                                        errorHandlingMode) {}

 protected:
  virtual DateTime parseDate(const std::string& date) const;
};

class FileDataSourceFormat4 : public FileDataSourceFormat6FieldsBase {
 public:
  FileDataSourceFormat4(
      const Info& info, const std::string& path, const std::string& ext,
      bool flatData,
      ErrorHandlingMode errorHandlingMode) throw(DataSourceException)
      : FileDataSourceFormat6FieldsBase(info, path, ext, format4, flatData,
                                        errorHandlingMode) {}
...
inline FileDataSource* FileDataSource::make(
    const Info& info, const std::string& path, const std::string& ext,
    Format format, bool flatData,
    ErrorHandlingMode errorHandlingMode) throw(DataSourceException) {
  switch (format) {
    case format1:
      return new FileDataSourceFormat1(info, path, ext, flatData,
                                       errorHandlingMode);
      break;
    case format2:
      return new FileDataSourceFormat2(info, path, ext, flatData,
                                       errorHandlingMode);
      break;
    case format3:
      return new FileDataSourceFormat3(info, path, ext, flatData,
                                       errorHandlingMode);
      break;
    case format4:
      return new FileDataSourceFormat4(info, path, ext, flatData,
                                       errorHandlingMode);
      break;
    default:
      throw DataSourceException(DATA_SOURCE_FORMAT_ERROR, "Unknown data format",
                                info.name());
      break;
  }
}

inline DateTime FileDataSourceFormat1::parseDate(
  ...
  if ((hour > 23) || (min > 59) || (sec > 59)) throw TimeException(time);
 ...
 * @exception DataSourceException
 */
inline const tradery::Bar* FileDataSourceFormat7FieldsBase::parseBarLine(
    const std::string& str) const throw(DataSourceException, BarException) {
  if (str.empty()) return 0;
  // comment line starts with // or # or $
  if (isCommentLine(str)) return 0;

  //	tradery::tsprint_debug( str );
  //	tradery::tsprint_debug( "\n" );
  ...
  return new Bar(parseDate(vs[0], vs[1]), strtod(vs[2].c_str(), &p),
                 strtod(vs[3].c_str(), &p), strtod(vs[4].c_str(), &p),
                 strtod(vs[5].c_str(), &p), atol(vs[6].c_str()));
}
...
inline const tradery::Bar* FileDataSourceFormat6FieldsBase::parseBarLine(
  ...
  char* p;
  return new Bar(parseDate(vs[0]), strtod(vs[1].c_str(), &p),
                 strtod(vs[2].c_str(), &p), strtod(vs[3].c_str(), &p),
                 strtod(vs[4].c_str(), &p), atol(vs[5].c_str()));
}
...
inline DataSource::DataXPtr FileDataSource::makeBars(
    const std::string& symbol, const std::string& ext,
    DateTimeRangePtr range) const throw(DataSourceException) {
...
//
  ...
  if (range != 0) COUT << range->toString() << std::endl;
#endif

  BarsPtr bars =
      tradery::createBars(name(), symbol, tradery::BarsAbstr::Type::stock,
                          24 * 3600, range, _errorHandlingMode);

  std::string fileName(
      FileName(_flatData).makePath(_path, symbol, addExtension(symbol, ext)));
  ...
  if (!_file) {
    ...
    try {
      ...
      bars->setDataLocationInfo(
          tradery::makeDataFileLocationInfo(fileName, p.start(), p.count()));
      ...
      return DataXPtr(new DataX(bars, getFileStamp(fileName)));
    } catch (const DataFileException) {
      throw DataSourceException(
          DATA_SOURCE_ERROR,
          std::string("Could not find any valid date in data file, likely due "
                      "to data wrong format "),
          name());
    } catch (const BarException&) {
      // retthrow bar exception, it will be caught in scheduler
      throw;
    } catch (const DateException& e) {
      throw DataSourceException(DATE_STRING_ERROR, symbol + " - " + e.message(),
                                name());
    } catch (const TimeException& e) {
      throw DataSourceException(TIME_STRING_ERROR, symbol + ", " + e.time(),
                                name());
    } catch (...) {
...
/*
DataSource::DataX* FileDataSource::makeTicks( const std::wstring& symbol, const
std::wstring& ext, const Range* range ) const throw( DataSourceException )
{
  // create an empty collection of bars
  // use a smart pointer (BarsIPtr) in case there is an exception, and the
collection needs to be deleted
  //TODO: determine the type and resolution, now hardcoded
  tradery::TicksPtr ticks( tradery::createTicks( name(), symbol, range ) );

        std::wstring fileName( makeFileName( symbol, ext ));
  // open the file with the data
  t_ifstream _file( fileName.c_str() );
  if ( !_file )
  {
      // error opening the file - throw exception
    throw DataSourceException( OPENING_TICKS_FILE_ERROR, fileName, name() );
  }
  else
  {
    // parse the file and populate the bars collection with bars
    parseTicks( ticks.get(), _file, range );
    // release and return the pointer
    // have to release so it won't be deleted by the smart pointer
    return new DataX( ticks.release(), getFileStamp( fileName ) );
  }
}
*/

inline const std::string FileDataSource::getFileStamp(
    const std::string& fileName) const {
  OutputBuffer ob;

  ob << "File data source: " << name() << ", session: " << sessionName()
     << ", getting file stamp";
  ...
  std::ostringstream o;
  o << lastWrite.dwHighDateTime << lastWrite.dwLowDateTime;
  return o.str();
}
